"""Define a function that takes an integer argument and returns a logical value true or false depending on if
...
def is_prime(num):
    if num in (0, 1) or num < 0 or not num % 2 or not num % 3:
        return num == 2
    for x in range(5, int(pow(num, 0.5)) + 1):
        if num % x == 0:
            return False
    return True
