...
def longest_palindrome(s):
    ...
    #  Проверяем крайний случай, что если вся строка - палиндром, значит она и будет самым длинным, вернём её длину
    if s == s[::-1]:
        return len(s)
    ...
    for i in range(len(s)):
        """
        length1 находит максимальную длину палиндрома, у которого центр - конкретный символ 
        'lorol' - (lo{r центр тут}ol)
        length2 находит максимальную длину палиндрома, когда мы берём за центр 'пространство' между двумя соседними
        символами 'baab' - (ba{центр как бы между 2 частями}ab)
        """
        length1 = expand_from_center(s, i, i)
        length2 = expand_from_center(s, i, i + 1)
        length_max = max(length1, length2)

        """Для того, чтобы не хранить в памяти все полученные палиндромы, а лишь самый длинный на текущий момент, будем 
